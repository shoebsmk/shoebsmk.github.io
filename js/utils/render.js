...
export function renderHero() {
  try {
    ...
    heroSection.innerHTML = `
      <h1 class="text-8xl md:text-9xl mb-4 bg-gradient-to-r from-[#14418A]/90 to-[#14418A]/90 bg-clip-text text-transparent">${escapeHtml(name)}</h1>
            ...
            <a href="${escapeUrl(project.url)}" target="_blank" rel="noopener noreferrer" class="glass-card overflow-hidden relative group cursor-pointer block p-6 flex flex-col">
              <div class="flex items-start gap-4 mb-4 min-h-[80px]">
                ...
                <div class="flex-1">
                  ...
                  <p class="text-[#14418A]/90 text-sm font-semibold">${escapeHtml(project.category || '')}</p>
                </div>
              </div>
      ...
      <div class="glass-card">
        <form id="contactForm" class="space-y-6">
          <div>
            ...
            <input type="text" id="name" name="name" ${form.name.required ? 'required' : ''} class="w-full px-4 py-3 rounded-lg border border-white/20 bg-white/10 text-[#2d3748] placeholder:text-[#2d3748]/50 focus:outline-none focus:border-[#14418A]/40 transition-colors duration-150" placeholder="${escapeHtml(form.name.placeholder || '')}">
          </div>
          <div>
